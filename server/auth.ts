...
async function insertSession(user:AuthUser,userAgent='') {
...
export async function createSession(res: Response, user: AuthUser, userAgent = '') {
  const {token,csrf}=await insertSession(user,userAgent);
  res.cookie(SESSION_COOKIE, token, cookieOptions(true));
  res.cookie(CSRF_COOKIE, csrf, cookieOptions(false));
  return csrf;
}

export async function createMobileSession(user:AuthUser,userAgent='') {
  const {token}=await insertSession(user,userAgent);
  return token;
}
...
export async function destroySession(req: AuthedRequest, res: Response) {
  ...
  const tokenHash=req.sessionTokenHash || (token ? hash(token) : undefined);
  if (tokenHash) await db.execute('DELETE FROM sessions WHERE token_hash=?', [tokenHash]);
...
export function requireCsrf(req: AuthedRequest, res: Response, next: NextFunction) {
  if (['GET','HEAD','OPTIONS'].includes(req.method) || req.authMode === 'bearer') return next();
